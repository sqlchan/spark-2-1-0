 ...
 * [[org.apache.spark.streaming.api.java.JavaPairDStream]].
  *
  * [[org.apache.spark.streaming.dstream.DStream]]的Java友好界面，Spark Streaming中的基本抽象，表示连续的数据流。
  * DStream可以从实时数据（例如，来自TCP套接字，Kafka，Flume等的数据）创建，
  * 也可以通过使用诸如`map`，`window`之类的操作转换现有DStream来生成。
  * 有关适用于键值对DStreams的操作，请参阅[[org.apache.spark.streaming.api.java.JavaPairDStream]]
 */
class JavaDStream[T](val dstream: DStream[T])(implicit val classTag: ClassTag[T])
  ...
  override def wrapRDD(rdd: RDD[T]): JavaRDD[T] = JavaRDD.fromRDD(rdd)  //包装RDD
  ...
  // 使用默认存储级别（MEMORY_ONLY_SER）保留此DStream的RDD
