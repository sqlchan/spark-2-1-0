 ...
 * to create instances of this class.
  * 抽象类，表示[[org.apache.spark. stream.dstream]的DStream转换' mapWithState '操作的所有规范。
  * PairDStreamFunctions pair DStream]] (Scala)或[[org.apache.spark.stream .api.java]。JavaPairDStream JavaPairDStream]](Java)。
  * 使用“org.apache.spark. stream.statespec .function()”工厂方法创建该类的实例。
 *
 * Example in Scala:
 * {{{
 *    // A mapping function that maintains an integer state and return a String
  *    一个映射函数，它维护一个整数状态并返回一个字符串
 *    def mappingFunction(key: String, value: Option[Int], state: State[Int]): Option[String] = {
 *      // Use state.exists(), state.get(), state.update() and state.remove() 来管理状态，并返回必要的字符串
...
sealed abstract class StateSpec[KeyType, ValueType, StateType, MappedType] extends Serializable {
   ...
   * Set the RDD containing the initial states that will be used by `mapWithState`
    * 设置包含“mapWithState”将使用的初始状态的RDD
   */
   ...
   * will be partitioned. Hash partitioning will be used.
    * 设置分区的数量，通过这些分区，“mapWithState”生成的状态RDDs将被分区。将使用哈希分区
   */
